...
function placeSnakeSegments(segments) {
  for (let i=0; i<segments.length; i++) {
    ...
    placeElem(segment, SNAKE_FIELD_ELEM, char, "snake" + extra, SQUARE_SIZE);
  }
}
...
function placeFruits(fruits) {
  for (let i=0; i<fruits.length; i++) {
    ...
    placeElem(position, SNAKE_FIELD_ELEM, FRUIT_CHAR, "fruit", SQUARE_SIZE);
  }
}
...
function placeLoadingSnake(func) {
  ...
  placeElem([0, 0], SNAKE_FIELD_ELEM, loadingSnakeHtml, "snake-loading", SQUARE_SIZE, func, true);
}

function placePauseText() {
  placeElem([0, 0], SNAKE_FIELD_ELEM, "Snake: PAUSED", "infobutton", SQUARE_SIZE, unPause);
...
function placeGameDetails() {
  placeElem([0, 2], SNAKE_FIELD_ELEM, `Play with arrow keys, mouse or touch.`, "infotext", SQUARE_SIZE);
  placeElem([0, 1], SNAKE_FIELD_ELEM, `Score: ${SNAKE_SCORE} | Snake Speed: ${Math.floor(SNAKE_CURRENT_SPEED * 100) / 100}`, "infotext", SQUARE_SIZE);
}
...
export var SNAKE_RUNNING = false;
export var SNAKE_PAUSED = false;
