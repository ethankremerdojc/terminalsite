...
var WORD_TO_GUESS = getRandomWord();
var GUESSED_LETTERS = [];
...
function drawLetterButtons() {
  for (let i=0; i<LETTERS.length; i++) {
    ...
    if (GUESSED_LETTERS.includes(letter)) {
      ...
      if (getBodyPartCount() < 6 && !doesPlayerWin()) {
        func = () => {
          selectLetter(letter) 
        }
      } else {
        buttonClass += " letter-button-game-over"
      }
    }

    placeElem([x + 1, y + 10], HANGMAN_FIELD_ELEM, letter, buttonClass, SQUARE_SIZE, func);
  }
}

function getBodyPartCount() {
  let count = 0;
  for (var letter of GUESSED_LETTERS) {
    if (!WORD_TO_GUESS.includes(letter)) {
      count ++;
    }
  }
...
function drawHangmanDude() {
  let currentBodyParts = getBodyPartCount();
  ...
  for (let i=0; i < currentBodyParts; i++) {
    let bodyPart = bodyParts[i];
    let bodyPartChar = bodyPart[0];
    ...
    if (["head", "body"].includes(bodyPartName)) {
      x = 1;
    } else if (bodyPartName.includes("left")) {
      x = 0;
    } else {
      x = 2;
    }

    placeElem([x + 3, y + 2], HANGMAN_FIELD_ELEM, bodyPartChar, `bodypart-${bodyPartName}`, SQUARE_SIZE);
  }
}
...
function drawGallows() {
  ...
  for (var part of parts) {
    ...
    placeElem([x + 4, y], HANGMAN_FIELD_ELEM, char, `gallows-part`, SQUARE_SIZE);
  }
}
...
function doesPlayerWin() {
  for (var char of WORD_TO_GUESS) {
    if (!GUESSED_LETTERS.includes(char)) {
      return false
    }
  }
  return true
...
function drawHangmanGame() {
  ...
  if (getBodyPartCount() == 6) {
    drawHangmanRestartButton();
  }

  if (doesPlayerWin()) {
    drawHangmanPlayAgainButton()
  }
...
export function initializeHangman() {
  GUESSED_LETTERS = [];
